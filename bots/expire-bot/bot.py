...
_Cleanup runs automatically every few minutes._""",
        ...
        "blind_room_notice": "I haven't been able to decrypt messages in this room for 24 hours "
            "({n} events from {senders} member(s)).\n"
            "To fix: kick me and invite me back. That will refresh the encryption keys. "
            "Retention settings will be preserved.\n"
            "Timer-based deletion keeps working in the background.",
    },
    "ru": {
...
_Очистка запускается автоматически каждые несколько минут._""",
        ...
        "blind_room_notice": "Я не могу расшифровать сообщения в этой комнате уже сутки "
            "({n} событий от {senders} участник(ов)).\n"
...
class ExpireBot:
    ...
    async def _check_blind_rooms(self):
        ...
        for room_id, h in list(self._room_health.items()):
            ...
            try:
                msg = await self._t(room_id, "blind_room_notice",
                                    n=len(recent_failed), senders=distinct)
