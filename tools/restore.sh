...
if [[ "$USE_LATEST" == true ]]; then
    ...
    [[ -n "$SNAPSHOT" ]] || {
        err "В $BACKUP_DIR нет снимков"
        exit 1
    }
fi
[[ -n "$SNAPSHOT" ]] || {
    err "Укажи снимок или --latest"
    exit 1
}
[[ -d "$SNAPSHOT" ]] || {
    err "Каталог снимка не найден: $SNAPSHOT"
    exit 1
}

DUMP="$SNAPSHOT/pgdumpall.sql.gz"
[[ -f "$DUMP" ]] || {
    err "Нет дампа БД в снимке: $DUMP"
    exit 1
}
if ! gzip -t "$DUMP" 2>/dev/null; then
    err "Дамп повреждён (gzip -t): $DUMP"
    exit 1
fi
...
[[ -f "$PG_ENV" ]] || {
    err "Не найден $PG_ENV - postgres не развёрнут?"
    exit 1
}
...
[[ "$ready" == true ]] || {
    err "$PG_CONTAINER не отвечает (pg_isready)"
    exit 1
}
...
if [[ "$RESTORE_CONFIG" == true ]]; then
    restore_cfg() {
        ...
        [[ -f "$arc" ]] || {
            warn "$name: $arc нет в снимке - пропуск"
            return 0
        }
