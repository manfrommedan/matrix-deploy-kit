...
> ⚠️ **Компромисс безопасности — прочти до включения.**
> Любой мост видит всю маршрутизируемую переписку **в открытом виде**, в обе
> стороны. Бридж расшифровывает Matrix (Megolm), упаковывает в протокол
> мессенджера (Signal-протокол WhatsApp и т.д.) — plaintext неизбежно живёт
> в процессе бриджа на сервере. Сквозного E2EE через мост **не существует** —
> таковы все puppet-мосты, это не настройка.
>
> Последствия: владелец root на хосте бриджа (ты сам, хостер при взломе VPS)
> может читать мостовую переписку. E2EE моста закрывает только Matrix-сегмент:
> в БД Synapse и на других серверах федерации комнаты-порталы хранятся
> шифротекстом.
>
> Практические правила:
> - чувствительные переписки — без бриджа (напрямую из приложения мессенджера);
> - высокие требования — выноси бридж на своё железо (не VPS-хостер);
> - ключи бриджа в его БД зашифрованы pickle-ключом, но сам файл БД + бэкап =
> ключ к переписке — храни бэкапы зашифрованными.
